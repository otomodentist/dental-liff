...
- `handleEvent(event)` — followイベント（管理者: リッチメニュー再設定 / 学生: 自動登録）・messageイベントのルーティング
- `handleAdminCommand(text, replyToken, timestamp)` — `[授業開始]` / `[出題]` / `[授業終了]` コマンド処理
...
**セッション管理**
- `startSession(subjectName, sessionNumber, timestamp, replyToken)` — LINEコマンドからの授業開始
- `endSession(replyToken)` — LINEコマンドからの授業終了
- `broadcastQuestion(question, modelAnswer, timestamp, replyToken)` — LINEコマンドからの出題・全員配信
- `adminStartSession(subjectName, sessionNumber)` — LIFF管理画面からの授業開始
- `adminEndSession()` — LIFF管理画面からの授業終了
- `adminBroadcast(question, modelAnswer)` — LIFF管理画面からの出題・全員配信
...
### 授業実施フロー（管理者 – LINEコマンド）

```
[授業開始] 科目名 回数  →  セッション作成 + 全学生にLINE通知
[出題] 問題文 / 模範解答  →  問題保存 + 全学生にLINE配信
[授業終了]  →  セッション終了 + 全学生にLINE通知
```
...
→ 科目選択 → 授業開始ボタン → adminStartSession()
→ 問題入力 → 出題ボタン → adminBroadcast()
→ 出題停止ボタン → pauseQuestionDelivery()（学生にLINE通知）
→ 授業終了ボタン → adminEndSession()
```
